...
def read_s3_data(context, s3_io_manager: S3Resource):
    ...
    time.sleep(90)
    s3_io_manager.get_client().get_object(
        Bucket="etl-dagster-data", Key="input_data.csv"
    )
    context.log.info("Client connection success")
    with open("data/input_data.csv", "wb") as f:
        s3_io_manager.get_client().download_fileobj(
            "etl-dagster-data", "input_data.csv", f
        )  # TODO just read data dont download it
...
def load_data(context, transformed_data, postgres_io_manager: SqlAlchemyClientResource):
    ...
    transformed_data.to_sql(
        "engineers",
        postgres_io_manager.create_engine(),
        if_exists="replace",
        index=False,
    )
